...
uint32_t Wheel(byte WheelPos) {
  ...
  if (WheelPos < third) {
    return strip.Color(WheelPos * 3 * brightness / 255, (full - WheelPos * 3)  * brightness / 255, 0);
  }
  else if (WheelPos < secondthird) {
    WheelPos -= third;
    return strip.Color((full - WheelPos * 3) * brightness / 255, 0, WheelPos * 3 * brightness / 255);
  }
  else {
    WheelPos -= secondthird;
    return strip.Color(0, WheelPos * 3 * brightness / 255, (full - WheelPos * 3) * brightness / 255);
  }
}
...
void set_matrix()
{
  ...
  for (int i = 0; i < MAX_X; i += 1) {
    ...
    int j = i; { ////for (int j = 0; j < MAX_Y; j += 1) {
...
void setup()
{
  ...
  ArduinoOTA.setHostname("WordClock V1.1");
